...
class LstmCLS(AMPCLS):
    ...
    def __init__(self, vocab_size, embedding_size, hidden_size, output_size,
        ...
        super(LstmCLS, self).__init__(**kwargs)
    ...
    def forward(self, input_ids, attention_mask):
        # input_ids is in shape [batch_size, max_len]
        seq_lens = torch.count_nonzero(attention_mask, dim=1).detach().cpu().numpy() #torch.count_nonzero(seqs-PAD_TOKEN_ID, dim=1).detach().cpu().numpy()
        x = self.word_embedder(input_ids)
...
class TransformerCLS(AMPCLS):
    ...
    def __init__(self, vocab_size, embedding_size, hidden_size, output_size, 
        ...
        encoder_layers = nn.TransformerEncoderLayer(embedding_size, nhead, hidden_size, dropout)
    ...
    def forward(self, input_ids, attention_mask):
        # input_ids is of shape [batch_size, max_len]
        # In transformer, we should first exchange first two dimensions.
        #ipdb.set_trace()
        x = torch.transpose(input_ids, 0, 1)
        src_key_padding_mask = 1 - attention_mask
